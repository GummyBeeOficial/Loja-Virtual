...
export default function AdminProductsEditPage() {
  ...
  const addVariant = () => {
    if (!newVariant.color_name || !newVariant.color_name.trim()) {
  ...
  const fetchInitialData = React.useCallback(async () => {
    ...
    try {
      ...
      if (productData) {
        // 1. Extrair tamanhos globais (pega da primeira variação que tiver tamanhos como base)
        let initialGlobalSizes: { size: string, stock: number }[] = [];
        const firstVariantWithSizes = (productData.product_variants || []).find((v: any) => v.product_sizes && v.product_sizes.length > 0);
        
        if (firstVariantWithSizes) {
          initialGlobalSizes = firstVariantWithSizes.product_sizes.map((s: any) => ({
            size: s.size,
            stock: s.stock
          }));
        }

        // 2. Map variants (PRESERVANDO O ID PARA SINCRONIZAÇÃO)
        let mappedVariants = (productData.product_variants || []).map((v: any) => ({
          id: v.id, // ID preservado para evitar delete/reinsert
        ...
        // FALLBACK: Se não houver variações mas houver tamanhos no formato antigo (coluna sizes no products)
        if (mappedVariants.length === 0 && productData.sizes && productData.sizes.length > 0) {
          console.log('DEBUG: Detectado formato antigo de tamanhos. Convertendo para variação padrão.');
          initialGlobalSizes = productData.sizes.map((size: string) => ({
            size: size,
            stock: 0 // Estoque inicial 0 para migração
          }));
          mappedVariants = [{
            variant_name: 'Padrão',
            color_name: 'Padrão',
        ...
        setGlobalSizes(initialGlobalSizes);
        setFormData({
  ...
  }, [productId, router, isStaff, role]);

  useEffect(() => {
    if (isStaff && (role === 'admin' || role === 'super_admin' || role === 'estoque') && productId) {
      fetchInitialData();
    }
  }, [isStaff, role, productId, fetchInitialData]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (hasPendingVariant()) {
      alert('Atenção: Você iniciou o preenchimento de uma variação (Cor/Estampa) mas não a confirmou. Clique no botão azul "Confirmar e Adicionar Variação" ou limpe os campos da nova variação antes de salvar o produto.');
      setSaving(false);
      return;
    }
    ...
    try {
      ...
      if (pError) {
        throw new Error(`Erro ao atualizar produto: ${pError.message}`);
      }

      // 2. Lógica de Sincronização de Variações (UPDATE ou INSERT)
      // REMOVIDO: Delete de variações e tamanhos para preservar integridade dos pedidos
      ...
      for (const variant of formData.variants) {
        ...
        if (variantId) {
          // UPDATE: Variação já existe, apenas atualizamos os dados
          ...
          if (uvError) {
            throw new Error(`Erro ao atualizar variação (${variant.color_name}): ${uvError.message}`);
          }
        } else {
          // INSERT: Nova variação adicionada na edição
          ...
          if (vError) {
            throw new Error(`Erro ao criar nova variação (${variant.color_name}): ${vError.message}`);
          }
          variantId = vData && vData[0]?.id;
        }

        if (!variantId) {
          throw new Error(`Erro crítico: Falha ao obter ID da variação ${variant.color_name}`);
        }

        // 3. Sincronizar Tamanhos para esta Variação (UPSERT)
        if (globalSizes.length > 0) {
          ...
          // UPSERT baseado na restrição única (product_variant_id, size)
          // Se o tamanho já existe para esta variação, atualiza o estoque. Senão, insere.
          const { error: sError } = await supabase
            .from('product_sizes')
            .upsert(sizesPayload, { onConflict: 'product_variant_id,size' });

          if (sError) {
            throw new Error(`Erro ao sincronizar tamanhos para ${variant.color_name}: ${sError.message}`);
          }
        }
      }
      ...
      <form id="product-form" onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white dark:bg-gray-900 p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 space-y-5">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Descrição Detalhada</label>
              <textarea 
                rows={8}
          ...
          <div className="bg-white dark:bg-gray-900 p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800">
            ...
            <div className="bg-gray-50 dark:bg-gray-800/50 p-5 rounded-xl border border-gray-100 dark:border-gray-800 mb-6 space-y-4">
              <h3 className="text-sm font-bold text-gray-700 dark:text-gray-300">Adicionar Nova Variação</h3>
